...
class AppModel extends \Illuminate\Database\Eloquent\Model
{
        ...
        if (is_array($filters)) {
            foreach ($filters as $model => $includes) {
                if (is_array($includes)) {
                    $query->with(array(
                        $model => function ($query) use ($includes, $model) {
                    ...
                    if (!empty($includes['whereHas'])) {
                        $match_operator = '>=';
                        if (!empty($includes['whereHas']['match_operator'])) {
                            $match_operator = $includes['whereHas']['match_operator'];
                            unset($includes['whereHas']['match_operator']);
                        }
                        $match_count = 1;
                        if (!empty($includes['whereHas']['match_count'])) {
                            $match_count = $includes['whereHas']['match_count'];
                            unset($includes['whereHas']['match_count']);
                        }
                        $query->whereHas($model, function ($query) use ($includes) {
                            $query = $this->setWhere($query, $includes['whereHas']);
                        }, $match_operator, $match_count);
                    }
                    foreach ($includes as $keyword => $valuefield) {
                        if (is_numeric($keyword)) {
                            $query = $query->with($model . '.' . $valuefield);
                        } elseif (!in_array($keyword, array(
                            'whereHas',
                            'where',
                            'limit',
                            'skip',
                ...
                $query = $query->where(function ($query) use ($key, $value) {
                    foreach ($value as $k => $v) {
                        if (in_array($k, array(
                            ...
                            $query = $this->setWhere($query, array(
                                $k => $v
                            ));
                        } else {
    ...
    public function setSubWhere($query, $key, $value)
    {
        if (is_array($value)) {
            ...
            if (in_array($operatorText, array(
                'eq',
                '=',
                'neq',
                'gt',
            ...
            } elseif (in_array($operatorText, array(
                'OR',
                'AND'
            ))) {
                $query = $this->setWhere($query, array(
