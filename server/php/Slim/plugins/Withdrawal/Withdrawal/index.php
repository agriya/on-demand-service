...
$app->GET('/api/v1/user_cash_withdrawals', function ($request, $response, $args) {
    ...
    try {
        ...
        $userCashWithdrawals = Models\UserCashWithdrawal::with($enabledIncludes)->Filter($queryParams)->paginate()->toArray();
...
$app->PUT('/api/v1/user_cash_withdrawals/{userCashWithdrawalsId}', function ($request, $response, $args) {
    ...
    if (!empty($userCashWithdrawal)) {
        if (!in_array($oldStatus, [\Constants\UserCashWithdrawStatus::Approved, \Constants\UserCashWithdrawStatus::Rejected])) {
            ...
            $userCashWithdrawal->save();
            if (!empty($userCashWithdrawal['user']['user_profile']['first_name']) || !empty($userCashWithdrawal['user']['user_profile']['last_name'])) {
                $username = $userCashWithdrawal['user']['user_profile']['first_name'].' '.$userCashWithdrawal['user']['user_profile']['last_name'];
            } else {
                $username = $userCashWithdrawal['user']['email']; 
            }            
...
$app->GET('/api/v1/money_transfer_accounts', function ($request, $response, $args) {
    ...
    try {
        ...
        if (!empty($queryParams['limit']) && $queryParams['limit'] == 'all') {
            ...
            $moneyTransferAccounts = Models\MoneyTransferAccount::with($enabledIncludes)->Filter($queryParams)->paginate()->toArray();
...
$app->GET('/api/v1/users/{userId}/money_transfer_accounts', function ($request, $response, $args) {
    ...
    try {
        ...
        if (!empty($queryParams['limit']) && $queryParams['limit'] == 'all') {
            ...
            $moneyTransferAccounts = Models\MoneyTransferAccount::where('user_id', $request->getAttribute('userId'))->Filter($queryParams)->paginate()->toArray();
            $data = $moneyTransferAccounts['data'];
            unset($moneyTransferAccounts['data']);
            $result = array(
                'data' => $data,
...
$app->POST('/api/v1/users/{userId}/money_transfer_accounts', function ($request, $response, $args) {
    ...
    if (empty($validationErrorFields)) {
        ...
        try {
            ...
            return renderWithJson($result, 'Account could not be added. Please, try again.', $e->getMessage(), 1,422);
        }
    } else {
        return renderWithJson($result, 'Account could not be added. Please, try again.', $validationErrorFields, 1,422);
    }
